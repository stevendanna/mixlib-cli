    ...
    def parse_options(argv=ARGV)
      ...
      @opt_parser = OptionParser.new do |opts|
        ...
        options.sort { |a, b| a[0].to_s <=> b[0].to_s }.each do |opt_key, opt_val|
          ...
          parse_block =
            Proc.new() do |c|
            # Workaround for CHEF-2641
            special_case = opt_val[:boolean] && opt_val.has_key?(:long) && ! opt_val[:long].match(/^--no-.*/).nil?
            config[opt_key] = if special_case
                                (opt_val[:proc] && opt_val[:proc].call(c)) || true
                              else
                                (opt_val[:proc] && opt_val[:proc].call(c)) || c
                              end
