...
describe Mixlib::CLI do
  ...
  describe "instance methods" do
    ...
    describe "parse_options" do
      ...
      it "should set the corresponding config value to true for boolean arguments even when long name has no in it" do
        TestCLI.option(:i_am_boolean, :long => "--no-way", :boolean => true)
        @cli = TestCLI.new
        @cli.parse_options([ '--no-way' ])
        @cli.config[:i_am_boolean].should == true
      end
